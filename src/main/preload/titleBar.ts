...
export const injectBrowserAction = () => {
  ...
  const browserAction = {
    ...
    async getState(
      partition: string
    ): Promise<{ activeTabId?: number; actions: any[] }> {
      const state = await invoke<any>('browserAction.getState', partition);
      state.actions.forEach((action: any) => actionMap.set(action.id, action));
    ...
    addObserver(partition: string) {
      let count = observerCounts.has(partition)
        ? observerCounts.get(partition)!
        : 0;
      count += 1;
  ...
  function mainWorldScript() {
    ...
    !customElements.get('browser-action') &&
...
.badge {
  ...
  white-space: nowrap;
      ...
      private stopObserving() {
        ...
        window.browserAction.removeObserver(
      ...
      private fetchState = async () => {
        try {
          await window.browserAction.getState(
            this.partition || DEFAULT_PARTITION
          );
        } catch {
          console.error(
      ...
      private update = (state: any) => {
        const tabId =
          typeof this.tab === 'number' && this.tab >= 0
            ? this.tab
            : state.activeTabId || -1;
    ...
    !customElements.get('browser-action-list') &&
